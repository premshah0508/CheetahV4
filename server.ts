...
/* ═══════════════════════════════════════════════════════════
   CONFIG
 ═══════════════════════════════════════════════════════════ */
const OWNER_KEY = process.env.OWNER_KEY || 'PremRivieraCullen';
...
async function startServer() {
        ...
        for (const comp of row.components) {
          ...
          const thread = async (n: number, delayMs: number) => {
            ...
            try {
              ...
              // Non-success — retry once after 30ms
              if (ev.succeeded) return;
          ...
          // 🚀 FIRE ALL THREADS INSTANTLY — zero logging during clicks
          const promises = threadsConfig.map((t, idx) => thread(idx + 1, t.delayMs));

          // After ALL clicks complete — flush detection + results + stats in one batch
          Promise.allSettled(promises).then(() => {
            // Detection log
            emit(sid, `[DETECTED] "${label}" — ${Date.now() - t0}ms`, 'info');

            // All thread result logs (success/fail)
            ...
            // Update session detection tracking
            sess.detectionCount++;
            sess.lastDetectionAt = Date.now();

            io.to(sid).emit('lifetimeStats', { ...lts });
  ...
  const LOGIN_TIMEOUT_MS = 45000;        // 45 second login timeout
